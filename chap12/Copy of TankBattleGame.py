...
class Tank(MySprite):
    ...
    def update(self,ticks):
        ...
        self.float_pos.x += self.velocity.x * 1.5  
        self.float_pos.y += self.velocity.y * 1.5
    ...
    def respawn_tank(self):
        ...
        self.float_pos = Point(random.randint(0,400), 50)
        self.health_points = 200
        self.draw_health_bar(0, 100, self.health_points, 100)
        
    def health(self):
        self.health_points -= 30
        
                    
    def draw_health_bar(self,x1 , y1, x2, y2):     
        pygame.draw.line(screen, self.color, (x1 ,y1  ), (x2 , y2), 20)
...
class EnemyTank(Tank):
    ...
    def respawn_tank(self):
        self.float_pos = Point(random.randint(50, 760), 50)
        self.health_points = 600
        self.draw_health_bar(self.health_points, 100, 800, 100)   
...
def game_init():
    ...
    enemy_tank.rotation = 200
    #create bullets
    bullets = list()
    
    player.draw_health_bar(0, 100, player.health_points, 100)
    enemy_tank.draw_health_bar(enemy_tank.health_points, 100, 800, 100 )
...
while True:
    ...
    if not game_over:
        ...
        e_angle = target_angle(-player.turret.X,-player.turret.Y,
            -crosshair.X - crosshair.frame_width/2,
            -crosshair.Y - crosshair.frame_height/2)
        
        
         
        # move enemy tanks turret so that it points opposite to the player turret
        
        enemy_tank.turret.rotation = e_angle
        #move tank
        player.update(ticks)
        ...
        for bullet in bullets:
            ...
            if bullet.owner == "player":
                if pygame.sprite.collide_rect(bullet, enemy_tank):
                    enemy_tank.health()                
                    bullet.alive = False
                if enemy_tank.health_points > 799:
                    player_score += 1
                    enemy_tank.respawn_tank()
