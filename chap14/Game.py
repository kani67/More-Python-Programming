...
def game_init():
    global screen, backbuffer, font1, font2, timer, inventory_list, str1
    pygame.init()
...
def playerCollision(stepx,stepy):
    global TILE_EMPTY,TILE_ROOM,TILE_HALL,dungeon,player,level, green
    ...
    if char == 29: #portal up
        ...
        if level > 1:
            ...
            message("you are on the first level!")
...
def player_inventory(item):
    global inventory_list, green

    if len(inventory_list) < 10:
        inventory_list.append(item)
    else:
        message("You are carrying too much stuff!! ", green)    

def player_remove_inventory(item):
    global inventory_list
...
def print_inventory():
    global inventory_list, str1

    str1 = ''.join(inventory_list)
    print_text(font2, 80, 590, str1)
         
             
def print_stats():
    global inventory_list
    
    print_text(font2, 0, 590, "inventory: ")
